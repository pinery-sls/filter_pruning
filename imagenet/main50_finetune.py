...
parser.add_argument('data',default="/home/lh/sls", metavar='DIR',
...
def validate(val_loader, model, criterion):
    ...
    savepath = os.path.join(args.save, "trainfse.txt")
    with open(savepath, "a") as fp:
        fp.write("prec@1: " + str(top1.val) + ", prec@5: " + str(top5.val) + "\n")

    return top1.avg

def save_checkpoint(state, is_best, filepath, name='checkpoint50fse.pth'):
    torch.save(state, os.path.join(filepath, name))
    if is_best:
        shutil.copyfile(os.path.join(filepath, name), os.path.join(filepath, 'model_best50fse.pth'))
